...
const debugLog = require('debug')('AmiSurrogateServer');
const errorLog = require('debug')('AmiSurrogateServer:error');
 ...
 * AmiSurrogateServer
 */
class AmiSurrogateServer extends EventEmitter{
    ...
    constructor(options){
        ...
        Object.assign(this, {
            _helloMessage: `Asterisk Surrogate Server ${meta.version}`,
    ...
    listen(port){
        return new Promise((resolve, reject) => {
            this._server
                .on('error', reject)
                .listen(port, () => {
                    debugLog(`AMI Surrogate Server listening on ${this._server.address().port} port`);
    ...
    _connectionHandler(clientSocket){
        ...
        clientSocket._eventStream.on('amiAction', action => {
            if(this._clients[clientSocket._key]){
                AmiSurrogateServer._actionHandler(action, clientSocket);
    ...
    _authHandler(action, clientSocket){
        ...
        if(actionName === 'ping'){
            AmiSurrogateServer._pingHandler(action, clientSocket);
            return;
        }
    ...
    static _actionHandler(action, clientSocket){
        ...
        if(actionName === 'logoff'){
            AmiSurrogateServer._logoffHandler(action, clientSocket);
        }
    }
...
module.exports = AmiSurrogateServer;